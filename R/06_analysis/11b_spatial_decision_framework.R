# =============================================================================
# Improved Spatial Decision Framework: FIA Design Scale Focus
# =============================================================================
# Improvements:
# 1. Focus on FIA reference scales (64kha big hex, 2.4kha sample design)
# 2. Vermont zoom for fine-scale visualization
# 3. Density plots instead of histograms
# 4. Publication-ready style matching scale_dependency_comprehensive.png
# =============================================================================
...
if (length(top_gaps) > 0) {
  ...
  p_distributions <- ggplot(dist_df, aes(x = dbh_cm, fill = dataset)) +
    ...
    # Add gap size annotation
    geom_text(
      data = gap_metrics %>% filter(common_name %in% top_gaps),
      aes(x = (fia_max + nefin_max) / 2, y = Inf, 
          label = sprintf("+%.0f cm\n%d trees", gap_cm, n_nefin_beyond_fia_max)),
      vjust = 1.3, size = 3, fontface = "bold", color = "darkred",
      inherit.aes = FALSE
    ) +
    ...
    labs(
      title = "DBH Distributions: Full View with Gap Zones Highlighted",
      subtitle = "Red zone = NEFIN extends beyond FIA maximum | Red line = FIA's absolute limit",
      x = "DBH (cm)",
      y = "Density",
      fill = "Dataset"
    )
  ...
  cat("Creating Plot 3B: Box plot comparison...\n")
  
  p_boxes <- ggplot(dist_df, aes(x = dataset, y = dbh_cm, fill = dataset)) +
    geom_violin(alpha = 0.6, width = 0.8) +
    geom_boxplot(width = 0.3, alpha = 0.8, outlier.alpha = 0.3) +
    # FIA maximum line
    geom_hline(
      data = gap_metrics %>% filter(common_name %in% top_gaps),
      aes(yintercept = fia_max),
      linetype = "dashed", color = "red", linewidth = 0.8
    ) +
    # NEFIN points beyond FIA max
    geom_point(
      data = dist_df %>% 
        left_join(gap_metrics %>% select(common_name, fia_max), by = "common_name") %>%
        filter(dataset == "NEFIN", dbh_cm > fia_max),
      aes(x = dataset, y = dbh_cm),
    ...
    labs(
      title = "Size Distributions with Extreme Outliers Highlighted",
      subtitle = "Red points = NEFIN trees beyond FIA's maximum | Dashed line = FIA limit",
      x = "Dataset",
      y = "DBH (cm)",
      fill = "Dataset"
    )
